##################################################
## Project: ASAP
## Script purpose: t-SNE calculation v3
...
#output_dataset_path <- "/col_attrs/TSNE_seurat"
#n_pcs <- 50 # Default
...
data.seurat[['pca']] <- CreateDimReducObject(embeddings = as.matrix(data.pca), key = 'PC_', assay = 'RNA')
...
# Run t-SNE
data.seurat <- RunTSNE(data.seurat, reduction = "pca", dim.embed = 2, dims = 1:n_pcs, verbose = F)

# Writing the UMAP in the loom file
data.loom <- open_with_lock(input_loom, "r+")
add_matrix_dataset(handle = data.loom, dataset_path = output_dataset_path, storage.mode_param = "double", dataset_object = t(data.seurat@reductions$tsne@cell.embeddings))
