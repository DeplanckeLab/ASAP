...
suppressPackageStartupMessages(require(batchelor)) # MNN
...
combatBatchCorrect <- function(dataTab, batchTab = NULL, covarsTab = NULL) {
  ...
  if (!is.null(batchTab)) {
    ...
    if (!is.null(covarsTab)) {
      ...
      result <- data.frame(ComBat(as.matrix(dataTab), batchVect, modelMatr))
    } else {
      result <- data.frame(ComBat(as.matrix(dataTab), batchVect))
    }
  }
  rownames(result) <- rownames(dataTab)
  colnames(result) <- colnames(dataTab)
  result
}
...
mnnBatchCorrect <- function(dataTab, batchTab = NULL, covarsTab = NULL, 
  ...
  result <- mnnCorrect(as.matrix(dataTab), batch = batchVect,  k = mnnK, sigma = mnnSigma)
  result <- data.frame(assay(result))
  
  # recover column/row names
  colnames(result) <- colnames(dataTab)
  rownames(result) <- rownames(dataTab)
...
#' for details. They should also be normalized within each data set to remove 
#' cell-specific biases in capture efficiency and sequencing depth. 
fastmnnBatchCorrect <- function(dataTab, batchTab = NULL, covarsTab = NULL, 
                            mnnK = 20, mnnNdist = 3, mnnD = 50) {
  # correction with only covariates isn't possible
  if (!is.null(covarsTab)) error.json("Removal of numerical covariates are not supported by MNN. Try using limma instead.")
  if (is.null(batchTab)) error.json("MNN works only if you supply at least one categorical covariate!")
  ...
  result <- fastMNN(as.matrix(dataTab), batch = batchVect,  k = mnnK, ndist = mnnNdist, d = mnnD)
  result <- data.frame(assay(result))
...
batchRemoveMethods <- c('limma', 'combat', 'mnn', 'fastmnn') 
...
if (std_method_name == 'fastmnn') {
  ...
  mnn_ndist <- 3
  if (!is.na(args[8])) {
    mnn_ndist <- as.integer(args[8])
  } else {
    data.warnings[[2]] <- list(name = "No n_dist for MNN was submitted, using default one (3)", description = "No n_dist for MNN was submitted, using default one (3)")
  }
  mnn_d <- 50
  if (!is.na(args[9])) {
    mnn_d <- as.integer(args[9])
  } else {
    data.warnings[[3]] <- list(name = "No d for MNN was submitted, using default one (50)", description = "No d for MNN was submitted, using default one (50)")
  }
}
...
if (length(data.batch) != 0) {
  ...
  message(paste0("Batch effect from: ",
                paste0(colnames(data.batch), collapse = ", "),
                ", will be removed"))
} else {
  data.batch <- NULL
}
if (!is.null(data.covars)) {
...
data.out <- switch(std_method_name, 
                   ...
                   "fastmnn" = fastmnnBatchCorrect(data.parsed, data.batch, 
                                           data.covars, mnn_k, mnn_ndist, mnn_d))
