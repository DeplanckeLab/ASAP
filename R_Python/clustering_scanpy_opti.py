...
if len(sys.argv) >= 8: metric = sys.argv[7])
...
def louvain(adjacency, resolution = 1.0, random_state = 0):
    import louvain
    print('running Louvain clustering using the "louvain" package of Traag (2017)')
...
print('7. Distance metric:', metric)
...
if method == 'louvain':
	clusters = louvain(_connectivities)
elif method == 'leiden':
	clusters = leiden(_connectivities)      
